...
from basedata.models import Material, Category
...
class OutItemInline(admin.TabularInline):
    model = OutItem
    fields = ('inventory', 'warehouse', 'cnt', 'measure', 'price' )
...
class InitItemInline(admin.TabularInline):
    model = InitItem
    fields = ('material', 'warehouse', 'cnt', 'measure', 'price',)
    # raw_id_fields = ['material']
...
class StockInAdmin(generic.BOAdmin):
    ...
    # raw_id_fields = ['po']
    # fields = (('code',), ('title',), ('po',), ('warehouse',), ('batch',), ('status', 'amount',))
    fields = (('code'), ('title'), ('po'), ('warehouse'),  ('status'), ('amount'))
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj and obj.status == 9:
            ...
            return ['code', 'title', 'po', 'warehouse', 'status']
        else:
...
class StockOutAdmin(generic.BOAdmin):
    ...
    list_display = ['code', 'title', 'status', 'out_time', 'out_amount']
    ...
    # )
    fields = (('code'),  ('title'),('status'),  ('amount'), ('user'))
...
class InitialInventoryAdmin(generic.BOAdmin):
    ...
    fields = ('code', 'title', 'status', 'amount', 'attach')
...
class InventoryAdmin(generic.BOAdmin):
    # list_display = ['material', 'warehouse', 'get_category_name', 'cnt',  'measure', 'price']
    list_display = ['material', 'warehouse',  'cnt', 'measure', 'price']
    # fields = ('material', 'measure', 'warehouse', 'cnt', 'price')
    search_fields = ['material__name', 'material__code']
    list_filter = ['warehouse', 'material__category__name', 'tp']

    # # 添加limit_choices_to属性
    # def formfield_for_foreignkey(self, db_field, request, **kwargs):
    #     if db_field.name == 'tp':
    #         kwargs['queryset'] = Inventory.objects.filter(tp__in=['10', '11'])
    #
    #     return super().formfield_for_foreignkey(db_field, request, **kwargs)

    def get_category_name(self, obj):
        return obj.material.category.name if obj.material.category else None
...
class WareReturnAdmin(generic.BOAdmin):
    ...
    fields = (('code',), ('title',), ('out',), ('amount',), ('status',))
    readonly_fields = ['status']
    # raw_id_fields = ['out']
