...
class MaterialAdmin(generic.BOAdmin):
    CODE_PREFIX = 'IT'
    CODE_NUMBER_WIDTH = 5
    list_display = ['code', 'name', 'spec', 'warehouse', 'measure', 'tp', 'category']
    list_display_links = ['code', 'name']
    list_filter = ['brand', 'tp', 'category']
    search_fields = ['code', 'name']
    fields = (
        ('code', 'barcode'),
        ('name',),
...
class EmployeeAdmin(generic.BOAdmin):
    ...
    search_fields = ['code', 'name', 'idcard']
    fieldsets = [
        (None, {'fields': [
            ('code', 'phone',),
            ('name', 'idcard',),
    ...
    def get_readonly_fields(self, request, obj=None):
        if request.user.is_superuser:
            ...
            return ['status', 'position', 'user']

    def parse_idcard(self, idcard):
        ...
        # return province, birthday, gender
        return birthday, gender

    def save_model(self, request, obj, form, change):
        if form.is_valid():
            idcard = form.cleaned_data.get('idcard', '')
            # province, birthday, gender = self.parse_idcard(idcard)
            birthday, gender = self.parse_idcard(idcard)
